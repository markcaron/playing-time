...
import type { ResetHalfEvent, ResetGameEvent, SavePlanEvent, EditLineupEvent, CancelPlanEvent, DeletePlanEvent, PlanHalfSwitchEvent, GameHalfSwitchedEvent, NavigateStatsEvent, TimerToggleEvent } from './pt-timer-bar.js';
...
export class PlayingTime extends LitElement {
  ...
  @state() accessor playerDisplayMode: 'number' | 'position' = 'number';
  ...
  #lastTickElapsed = 0;
  /** Updated each tick from positionTracker.getAllPositionTimes() */
  #positionTimes: Record<string, Partial<Record<string, number>>> = {};
  ...
  #startPolling() {
    ...
    this.#timerInterval = setInterval(() => {
      ...
      if (delta > 0) {
        ...
        const slots = this.fieldPlayers.map((fp, i) => ({ playerId: fp.id, slotIndex: i }));
        this.#positionTracker.accumulate(slots, this.formation, delta);
        const positionTimes = this.#positionTracker.getAllPositionTimes();
        this.roster = this.roster.map(p => ({ ...p, positionTimes: positionTimes[p.id] ?? p.positionTimes }));
      }
      this.requestUpdate();
    }, 1000);
    this.requestUpdate();
  ...
  #onTimerToggle(_e: TimerToggleEvent) {
  ...
  #loadTeam(teamId: string) {
    ...
    this.playerDisplayMode = team.playerDisplayMode ?? 'number';
  ...
  #onDisplayModeChanged(e: Event) {
    const mode = (e as CustomEvent).detail?.mode;
    if (mode === 'number' || mode === 'position') {
      this.playerDisplayMode = mode;
      this.#saveState();
    }
  }
  ...
  render() {
    ...
    if (this.currentView === 'settings') {
      return html`
        <pt-settings-view
          ...
          @display-mode-changed="${this.#onDisplayModeChanged}"
    ...
    if (this.currentView === 'stats') {
      // roster entries carry positionTimes from PositionTracker
      return html`
        <pt-stats-view
          .teamName="${this.teamName}"
          .roster="${this.roster}"
    ...
    return html`
      ...
      <div class="app-container">
        ...
        <pt-timer-bar
          ...
          .half2Started="${this.half2Started}"
          .timeDisplayFormat="${this.timeDisplayFormat}"
          @timer-toggle="${this.#onTimerToggle}"
